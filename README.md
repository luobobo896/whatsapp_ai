...
```
├── cmd/          # Go CLI 入口（server, db-create, db-migrate, bootstrap-admin, import-legacy）
├── internal/     # Go 业务代码（auth, tenants, members, operations, platform, audit, legacy）
├── web/          # 前端嵌入层，go:embed dist → http.Handler
├── migrations/   # PostgreSQL 迁移，SHA-256 checksum
├── frontend/     # React/Vite 管理端（独立 pnpm 项目，构建输出到 ../web/dist）
├── config/       # 遗留数据文件
├── docs/         # 文档
└── scripts/      # Shell 脚本
...
```bash
cd frontend && pnpm install && pnpm run build   # 输出到 ../web/dist/
```

本地需要独立调试前端时可运行 `cd frontend && pnpm run dev`，Vite 会将 `/api` 和 `/health` 代理到 `127.0.0.1:8790`。此时后端的 `APP_ORIGIN` 需要设为 Vite 地址（默认 `http://localhost:5173`），保证写请求通过 Origin/CSRF 校验。生产和日常启动不需要单独运行前端进程。
